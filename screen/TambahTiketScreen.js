...
function TambahTiketScreen({navigation}) {
...
//   const [judul, setJudul] = useState('');
  ...
  const getKategori = async() => {
    const { data, error } = await supabase
                              .from('kereta')
                              .select('*')
                              .order('nama_kereta', {ascending:true});
    setDataPicker(data);
  }
  ...
  return (
    <>
      <Appbar.Header>
        ...
        <Appbar.Content title="Tambah Tiket" />
      </Appbar.Header>
      ...
      >
        ...
        {dataPicker.map((row) => 
          <Picker.Item label={row.nama_kereta} value={row.id} />
        )}
      </Picker>
...
export default TambahTiketScreen;